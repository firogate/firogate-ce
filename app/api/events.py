...
Browser SSE limits:
  ...
  most proxies serve HTTP/2 no practical limit issue.
...
from app.services.event_bus import EventBus, _Entry
from app.api.users import get_current_user
from loguru import logger

router = APIRouter(prefix="/api/events", tags=["events"])

HEARTBEAT_INTERVAL  = 25    # seconds, survives a typical proxy read timeout
...
async def _stream(
    ...
    try:
        ...
        # Closes the race between "browser opens this connection" and
        # "subscribed to the pub/sub channel": if a terminal event was
        # published in that gap, the subscriber above never saw it and
        # the queue stays empty forever. Re-checking current state right
        # after subscribing (not before) guarantees no publish in between
        # can be lost.
        if recheck is not None:
            late_event = await recheck()
            if late_event is not None:
                yield _data(late_event["type"], {k: v for k, v in late_event.items() if k != "type"})
                logger.info(f"[sse] term  label={label} event={late_event['type']} (caught on subscribe)")
                return

        # Client always gets current state on connect
        if initial:
...
def _response(gen) -> StreamingResponse:
    return StreamingResponse(gen, media_type="text/event-stream", headers={
        ...
        "X-Accel-Buffering": "no",       # disable proxy buffering (if supported)
...
async def payment_stream(
    ...
    # Mandatory: prevents streaming another merchant's payment status via a
    # guessed/leaked payment_id with no token.
    token = request.query_params.get("t", "")
    ts    = p.created_at.isoformat() if p.created_at else ""
    if not token or not verify_checkout_token(payment_id, ts, token):
    ...
    # Fresh snapshot - client always syncs to DB truth on every connect/reconnect
    initial = {
        "type":                   "payment.status",
        "payment_id":             payment_id,
        "status":                 str(p.status),
    ...
    async def _recheck():
        ...
        async with AsyncSessionLocal() as fresh_db:
            r = await fresh_db.execute(select(Payment).where(Payment.id == payment_id))
            fresh_p = r.scalar_one_or_none()
            if fresh_p and fresh_p.status in _term_map:
                return {
                    "type":          _term_map[fresh_p.status],
                    "payment_id":    payment_id,
                    "status":        str(fresh_p.status),
                    "txid":          fresh_p.txid,
                    "confirmations": fresh_p.confirmations,
                    "confirmed_at":  fresh_p.confirmed_at.isoformat() if fresh_p.confirmed_at else None,
                }
        return None
...
async def merchant_stream(
    ...
    today_count, today_received = today_res.one()

    initial = {
        "type":              "dashboard.snapshot",
        "today_count":       int(today_count or 0),
        "today_received":    float(today_received or 0),
        "recent":            [
            {
                "id":              str(px.id),
                "status":          str(px.status),
                "amount_firo":     float(px.amount_firo or 0),
                "amount_received": float(px.amount_received or 0),
