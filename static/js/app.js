...
const IC = {
  ...
  storefront:`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 9l1.5-5h15L21 9"/><path d="M3 9a2 2 0 004 0 2 2 0 004 0 2 2 0 004 0 2 2 0 004 0"/><path d="M5 9v10a1 1 0 001 1h12a1 1 0 001-1V9"/><path d="M9 21v-6h6v6"/></svg>`,
...
const API = {
  ...
  _refFailAt:0,
  async _ref(){
    if(Date.now()-API._refFailAt<15000)return false;
    if(API._refP)return API._refP;
    API._refP=(async()=>{
      try{
        ...
        if(r.ok){API._refFailAt=0}else{API._refFailAt=Date.now()}
        return r.ok;
      }catch{API._refFailAt=Date.now();return false}
      finally{API._refP=null}
    })();
    return API._refP;
  },
  ...
  // Like .get(), but also returns response headers — for endpoints that
  // expose pagination metadata (e.g. X-Total-Count) outside the JSON body,
  // since the body itself is a documented public-API contract that can't
  // change shape. Existing API.get() callers are untouched.
  async getWithHeaders(p){
    const r=await API._fetch('GET',p);
    if(!r)return{error:'Unauthorized'};
    const d=await r.json().catch(()=>({}));
    if(!r.ok){
      if (d && typeof d.detail === 'object' && d.detail !== null) {
